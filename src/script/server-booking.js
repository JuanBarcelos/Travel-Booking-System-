...
export const createBooking = async (formData) => {
    try {
        ...
        if (response?.success) {
            ...
            return { success: true, message: "Reserva cadastrada com sucesso!" };
        } else {
            return { success: false, message: "Erro ao cadastrar reserva no servidor." };
        }
    } catch (error) {
...
export const updateBooking = async (id, formData) => {
    try {
        ...
        if (response?.success) {
            ...
            return { success: false, message: "Erro ao atualizar reserva no servidor." };
        }
    } catch (error) {
...
const sendPostUserRequest = async (data) => {
    const response = await axios.post("https://sheetdb.io/api/v1/2fmj1ybi9hf0s/?sheet=Login", {  data });
    return response?.data ? { success: true, message: "Usuário cadastrado com sucesso!" } : { success: false, message: "Erro ao cadastrar usuário." };
}

// Função para enviar requisição POST
const sendPostRequest = async (data) => {
    try {
        const response = await axios.post("https://sheetdb.io/api/v1/2fmj1ybi9hf0s", { data });
        return response?.data ? { success: true, message: "Reserva cadastrada com sucesso!" } : { success: false };
    } catch (error) {
...
const sendGetCodeRequest = async (code) => {
    const response = await axios.get(`https://sheetdb.io/api/v1/2fmj1ybi9hf0s/search?sheet=Autenticar&CODE=${code}`);
    return response?.data ? { success: true, message: "Autenticado com sucesso!" } : { success: false, message: "Código invalido." };
};
